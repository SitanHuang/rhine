...
MAP_DATA.forEach((x, row) => (x.forEach((v, col) => {
  ...
  if (v.pt.adjacentNotToPlayer(v.pt.owner) > 0 || v.terrain == 'U' || v.terrain == 'P')
    if (v.owner == 0)
      v.divisions = Array(v.terrain == 'P' ? 5 : Math.ceil(Math.random() * 5)).fill(0).map(() => (new Division(v.owner, `${++divisions}th Infantry Division`, pt(row, col), new Template(6500, 15, 5))))
    else
...
// Shanghai
pt(27, 46).prov.divisions = Array(20).fill(0).map(() => (new Division(1, 'Infantry Division', pt(27, 46), new Template(23000, 35, 25))))
  .concat(Array(10).fill(0).map(() => (new Division(1, 'Tank Division', pt(27, 46), new Template(20000, 10, 40)))));
// Nanjing
