...
MAP_DATA.forEach((x, row) => (x.forEach((v, col) => {
  ...
  if (v.pt.adjacentNotToPlayer(v.pt.owner) > 0 || v.terrain == 'U' || v.terrain == 'P')
    if (v.owner == 0)
      v.divisions = Array(v.terrain == 'P' ? 2 : 5).fill(0).map(() => (new Division(v.owner, 'Infantry Division', pt(row, col), new Template(7000, 17, 5))))
    else
...
// Shanghai
pt(27, 46).prov.divisions = Array(35).fill(0).map(() => (new Division(1, 'Infantry Division', pt(27, 46), new Template(10000, 20, 15))))
