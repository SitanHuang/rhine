var WWII1936Owners = `
00000000000000000000000000000000000000000000000000002200222222222222222222222222222222222|
00000000000000000000000000000000000000000000000000000200222222222222222222222222222222222|
00000000000000000000000000000000000000000000000000022220222222222222222222222222222222222|
...
function load1936Scenario() {
  clearData();
  // ============== map =================
  MAP_DATA = WWIITerrains1936.map((x, row) => (x.split('').map((v, col) => {
    let isBorder = WWII1936Owners[row][col] == '#';
    let o = isBorder ? 2 : parseInt(WWII1936Owners[row][col]);
    let slots = [];
    let terrain = '@';
    if (v == 'c'){
      terrain = 'U';
    ...
    if (isBorder) {
      ...
      return {terrain: ' ', owner: 2, slots: [], divisions: [], pt: pt(row, col)};
    } else {
      return _col;
    }
  })));
...
//   let germanArtillery = new Template(8500, 2, 40, 'Anti-Tank Regiment', 1, 7);
  ...
  let soviet = new Template(18800, 19, 6, 'Rifle Division', 2, 2);
  let sovietMilitia = new Template(11000, 5, 1, 'Militia Division', 0.1, 0.1);
  ...
  MAP_DATA.forEach((x, row) => (x.forEach((v, col) => {
    ...
    if (v.owner == 0) {
      ...
      if (v.owner == 1 && Math.random() > 0.9) {
        let type = Math.random() < 0.8 ? (Math.random() > 0.5 ? german : germanSS) : germanPanzer;
        let num = 0;
        v.divisions = Array(1 + num).fill(0).map(() => {
        ...
        let type = Math.random() < 0.8 ? soviet : (Math.random() < 0.05 ? sovietMech : sovietMilitia);
  ...
  diplomacy_change(PLAYERS[3].playerID, PLAYERS[2].playerID, {status: 'PACT'});
...
function trigger_german_ai_attack1936(num) {
  ...
  if (PLAYERS[3] && diplomacy_get(1,3).status == 'WAR' && diplomacy_get(2,3).status != 'WAR' &&
      MAP_DATA[15][47].owner == 1 && PLAYERS[3].divisionMen <= 100000) {
    // if warsaw taken by germany, then after 9/20, otherwise, WILL NOT HAPPEN (edit: NOT after 11/1)
  ...
  if (diplomacy_get(1,2).status == 'WAR') {
    if (num == 0) {
      ...
      MAP_DATA.forEach(r => r.forEach(c => {
        if (c && c.owner == 2){c.divisions.forEach(x => {airStrike(x);x.morale = 0.1;x.entrench=0.1;x.supply=0.1;})}
      }));
